...
function write(cobj::Algorithm, fs::Ptr{FileStorage}, name::String)
	return cpp_to_julia(jlopencv_cv_cv_Algorithm_cv_Algorithm_write(julia_to_cpp(cobj),julia_to_cpp(fs),julia_to_cpp(name)))
end
write(cobj::Algorithm, fs::Ptr{FileStorage}; name::String = String()) = write(cobj, fs, name)
...
detect(cobj::cv_Ptr{T}, edges::Image, dx::Image, dy::Image; positions::Image = Mat(), votes::Image = MatOP) where {T <: GeneralizedHough} = detect(cobj, edges, dx, dy, positions, votes)

function detect(cobj::cv_Ptr{T}, edges::UMat, dx::UMat, dy::UMat, positions::UMat, votes::UMat) where {T <: GeneralizedHough}
	return cpp_to_julia(jlopencv_cv_cv_GeneralizedHough_cv_GeneralizedHough_detect(julia_to_cpp(cobj),julia_to_cpp(edges),julia_to_cpp(dx),julia_to_cpp(dy),julia_to_cpp(positions),julia_to_cpp(votes)))
end
detect(cobj::cv_Ptr{T}, edges::UMat, dx::UMat, dy::UMat; positions::UMat = UMat(), votes::UMat = UMatOP) where {T <: GeneralizedHough} = detect(cobj, edges, dx, dy, positions, votes)
...
function write(cobj::cv_Ptr{T}, fs::Ptr{FileStorage}, name::String) where {T <: Feature2D}
	return cpp_to_julia(jlopencv_cv_cv_Feature2D_cv_Feature2D_write(julia_to_cpp(cobj),julia_to_cpp(fs),julia_to_cpp(name)))
end
write(cobj::cv_Ptr{T}, fs::Ptr{FileStorage}; name::String = String()) where {T <: Feature2D} = write(cobj, fs, name)
...
function write(cobj::cv_Ptr{T}, fs::Ptr{FileStorage}, name::String) where {T <: DescriptorMatcher}
...
write(cobj::cv_Ptr{T}, fs::Ptr{FileStorage}; name::String = String()) where {T <: DescriptorMatcher} = write(cobj, fs, name)
...
apply(cobj::cv_Ptr{T}, image::Image; fgmask::Image = Mat(), learningRate::Float64 = -1) where {T <: BackgroundSubtractor} = apply(cobj, image, fgmask, learningRate)

function apply(cobj::cv_Ptr{T}, image::UMat, fgmask::UMat, learningRate::Float64) where {T <: BackgroundSubtractor}
	return cpp_to_julia(jlopencv_cv_cv_BackgroundSubtractor_cv_BackgroundSubtractor_apply(julia_to_cpp(cobj),julia_to_cpp(image),julia_to_cpp(fgmask),julia_to_cpp(learningRate)))
end
apply(cobj::cv_Ptr{T}, image::UMat; fgmask::UMat = UMat(), learningRate::Float64 = -1) where {T <: BackgroundSubtractor} = apply(cobj, image, fgmask, learningRate)
...
getBackgroundImage(cobj::cv_Ptr{T}; backgroundImage::UMat = UMat()) where {T <: BackgroundSubtractor} = getBackgroundImage(cobj, backgroundImage)
...
apply(cobj::cv_Ptr{T}, image::Image; fgmask::Image = Mat(), learningRate::Float64 = -1) where {T <: BackgroundSubtractorMOG2} = apply(cobj, image, fgmask, learningRate)

function apply(cobj::cv_Ptr{T}, image::UMat, fgmask::UMat, learningRate::Float64) where {T <: BackgroundSubtractorMOG2}
	return cpp_to_julia(jlopencv_cv_cv_BackgroundSubtractorMOG2_cv_BackgroundSubtractorMOG2_apply(julia_to_cpp(cobj),julia_to_cpp(image),julia_to_cpp(fgmask),julia_to_cpp(learningRate)))
end
apply(cobj::cv_Ptr{T}, image::UMat; fgmask::UMat = UMat(), learningRate::Float64 = -1) where {T <: BackgroundSubtractorMOG2} = apply(cobj, image, fgmask, learningRate)
...
detectMultiScale(cobj::CascadeClassifier, image::Image; scaleFactor::Float64 = 1.1, minNeighbors::Int32 = 3, flags::Int32 = 0, minSize::Size = SizeOP, maxSize::Size = SizeOP) = detectMultiScale(cobj, image, scaleFactor, minNeighbors, flags, minSize, maxSize)
...
detectMultiScale(cobj::CascadeClassifier, image::UMat; scaleFactor::Float64 = 1.1, minNeighbors::Int32 = 3, flags::Int32 = 0, minSize::Size = SizeOP, maxSize::Size = SizeOP) = detectMultiScale(cobj, image, scaleFactor, minNeighbors, flags, minSize, maxSize)
...
detectMultiScale(cobj::CascadeClassifier, image::Image; scaleFactor::Float64 = 1.1, minNeighbors::Int32 = 3, flags::Int32 = 0, minSize::Size = SizeOP, maxSize::Size = SizeOP, outputRejectLevels::Bool = false) = detectMultiScale(cobj, image, scaleFactor, minNeighbors, flags, minSize, maxSize, outputRejectLevels)
...
detectMultiScale(cobj::CascadeClassifier, image::UMat; scaleFactor::Float64 = 1.1, minNeighbors::Int32 = 3, flags::Int32 = 0, minSize::Size = SizeOP, maxSize::Size = SizeOP, outputRejectLevels::Bool = false) = detectMultiScale(cobj, image, scaleFactor, minNeighbors, flags, minSize, maxSize, outputRejectLevels)
...
detect(cobj::HOGDescriptor, img::Image; hitThreshold::Float64 = 0, winStride::Size = SizeOP, padding::Size = SizeOP, searchLocations::Array{Point{Int32}, 1} = stdggvectoriPointkOP) = detect(cobj, img, hitThreshold, winStride, padding, searchLocations)
...
detect(cobj::HOGDescriptor, img::UMat; hitThreshold::Float64 = 0, winStride::Size = SizeOP, padding::Size = SizeOP, searchLocations::Array{Point{Int32}, 1} = stdggvectoriPointkOP) = detect(cobj, img, hitThreshold, winStride, padding, searchLocations)
...
detectMultiScale(cobj::HOGDescriptor, img::Image; hitThreshold::Float64 = 0, winStride::Size = SizeOP, padding::Size = SizeOP, scale::Float64 = 1.05, finalThreshold::Float64 = 2.0, useMeanshiftGrouping::Bool = false) = detectMultiScale(cobj, img, hitThreshold, winStride, padding, scale, finalThreshold, useMeanshiftGrouping)
...
detectMultiScale(cobj::HOGDescriptor, img::UMat; hitThreshold::Float64 = 0, winStride::Size = SizeOP, padding::Size = SizeOP, scale::Float64 = 1.05, finalThreshold::Float64 = 2.0, useMeanshiftGrouping::Bool = false) = detectMultiScale(cobj, img, hitThreshold, winStride, padding, scale, finalThreshold, useMeanshiftGrouping)
...
open(cobj::VideoCapture, filename::String; apiPreference::Int32 = CAP_ANY) = open(cobj, filename, apiPreference)
...
open(cobj::VideoCapture, index::Int32; apiPreference::Int32 = CAP_ANY) = open(cobj, index, apiPreference)
...
retrieve(cobj::VideoCapture; image::Image = Mat(), flag::Int32 = 0) = retrieve(cobj, image, flag)

function retrieve(cobj::VideoCapture, image::UMat, flag::Int32)
	return cpp_to_julia(jlopencv_cv_cv_VideoCapture_cv_VideoCapture_retrieve(julia_to_cpp(cobj),julia_to_cpp(image),julia_to_cpp(flag)))
end
retrieve(cobj::VideoCapture; image::UMat = UMat(), flag::Int32 = 0) = retrieve(cobj, image, flag)
...
read(cobj::VideoCapture; image::UMat = UMat()) = read(cobj, image)
...
add(src1::Image, src2::Image; dst::Image = Mat(), mask::Image = MatOP, dtype::Int32 = -1) = add(src1, src2, dst, mask, dtype)

function add(src1::UMat, src2::UMat, dst::UMat, mask::UMat, dtype::Int32)
	return cpp_to_julia(jlopencv_cv_cv_add(julia_to_cpp(src1),julia_to_cpp(src2),julia_to_cpp(dst),julia_to_cpp(mask),julia_to_cpp(dtype)))
end
add(src1::UMat, src2::UMat; dst::UMat = UMat(), mask::UMat = UMatOP, dtype::Int32 = -1) = add(src1, src2, dst, mask, dtype)
...
subtract(src1::Image, src2::Image; dst::Image = Mat(), mask::Image = MatOP, dtype::Int32 = -1) = subtract(src1, src2, dst, mask, dtype)

function subtract(src1::UMat, src2::UMat, dst::UMat, mask::UMat, dtype::Int32)
	return cpp_to_julia(jlopencv_cv_cv_subtract(julia_to_cpp(src1),julia_to_cpp(src2),julia_to_cpp(dst),julia_to_cpp(mask),julia_to_cpp(dtype)))
end
subtract(src1::UMat, src2::UMat; dst::UMat = UMat(), mask::UMat = UMatOP, dtype::Int32 = -1) = subtract(src1, src2, dst, mask, dtype)
...
norm(src1::Image; normType::Int32 = NORM_L2, mask::Image = MatOP) = norm(src1, normType, mask)

function norm(src1::UMat, normType::Int32, mask::UMat)
	return cpp_to_julia(jlopencv_cv_cv_norm(julia_to_cpp(src1),julia_to_cpp(normType),julia_to_cpp(mask)))
end
norm(src1::UMat; normType::Int32 = NORM_L2, mask::UMat = UMatOP) = norm(src1, normType, mask)
...
norm(src1::Image, src2::Image; normType::Int32 = NORM_L2, mask::Image = MatOP) = norm(src1, src2, normType, mask)

function norm(src1::UMat, src2::UMat, normType::Int32, mask::UMat)
	return cpp_to_julia(jlopencv_cv_cv_norm(julia_to_cpp(src1),julia_to_cpp(src2),julia_to_cpp(normType),julia_to_cpp(mask)))
end
norm(src1::UMat, src2::UMat; normType::Int32 = NORM_L2, mask::UMat = UMatOP) = norm(src1, src2, normType, mask)
...
batchDistance(src1::Image, src2::Image, dtype::Int32; dist::Image = Mat(), nidx::Image = Mat(), normType::Int32 = NORM_L2, K::Int32 = 0, mask::Image = MatOP, update::Int32 = 0, crosscheck::Bool = false) = batchDistance(src1, src2, dtype, dist, nidx, normType, K, mask, update, crosscheck)
...
batchDistance(src1::UMat, src2::UMat, dtype::Int32; dist::UMat = UMat(), nidx::UMat = UMat(), normType::Int32 = NORM_L2, K::Int32 = 0, mask::UMat = UMatOP, update::Int32 = 0, crosscheck::Bool = false) = batchDistance(src1, src2, dtype, dist, nidx, normType, K, mask, update, crosscheck)
...
normalize(src::Image, dst::Image; alpha::Float64 = 1, beta::Float64 = 0, norm_type::Int32 = NORM_L2, dtype::Int32 = -1, mask::Image = MatOP) = normalize(src, dst, alpha, beta, norm_type, dtype, mask)

function normalize(src::UMat, dst::UMat, alpha::Float64, beta::Float64, norm_type::Int32, dtype::Int32, mask::UMat)
	return cpp_to_julia(jlopencv_cv_cv_normalize(julia_to_cpp(src),julia_to_cpp(dst),julia_to_cpp(alpha),julia_to_cpp(beta),julia_to_cpp(norm_type),julia_to_cpp(dtype),julia_to_cpp(mask)))
end
normalize(src::UMat, dst::UMat; alpha::Float64 = 1, beta::Float64 = 0, norm_type::Int32 = NORM_L2, dtype::Int32 = -1, mask::UMat = UMatOP) = normalize(src, dst, alpha, beta, norm_type, dtype, mask)
...
mulTransposed(src::Image, aTa::Bool; dst::Image = Mat(), delta::Image = MatOP, scale::Float64 = 1, dtype::Int32 = -1) = mulTransposed(src, aTa, dst, delta, scale, dtype)

function mulTransposed(src::UMat, aTa::Bool, dst::UMat, delta::UMat, scale::Float64, dtype::Int32)
	return cpp_to_julia(jlopencv_cv_cv_mulTransposed(julia_to_cpp(src),julia_to_cpp(aTa),julia_to_cpp(dst),julia_to_cpp(delta),julia_to_cpp(scale),julia_to_cpp(dtype)))
end
mulTransposed(src::UMat, aTa::Bool; dst::UMat = UMat(), delta::UMat = UMatOP, scale::Float64 = 1, dtype::Int32 = -1) = mulTransposed(src, aTa, dst, delta, scale, dtype)
...
boxFilter(src::Image, ddepth::Int32, ksize::Size; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, normalize::Bool = true, borderType::Int32 = BORDER_DEFAULT) = boxFilter(src, ddepth, ksize, dst, anchor, normalize, borderType)

function boxFilter(src::UMat, ddepth::Int32, ksize::Size, dst::UMat, anchor::Point{Int32}, normalize::Bool, borderType::Int32)
	return cpp_to_julia(jlopencv_cv_cv_boxFilter(julia_to_cpp(src),julia_to_cpp(ddepth),julia_to_cpp(ksize),julia_to_cpp(dst),julia_to_cpp(anchor),julia_to_cpp(normalize),julia_to_cpp(borderType)))
end
boxFilter(src::UMat, ddepth::Int32, ksize::Size; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, normalize::Bool = true, borderType::Int32 = BORDER_DEFAULT) = boxFilter(src, ddepth, ksize, dst, anchor, normalize, borderType)
...
sqrBoxFilter(src::Image, ddepth::Int32, ksize::Size; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSGTXP, normalize::Bool = true, borderType::Int32 = BORDER_DEFAULT) = sqrBoxFilter(src, ddepth, ksize, dst, anchor, normalize, borderType)

function sqrBoxFilter(src::UMat, ddepth::Int32, ksize::Size, dst::UMat, anchor::Point{Int32}, normalize::Bool, borderType::Int32)
	return cpp_to_julia(jlopencv_cv_cv_sqrBoxFilter(julia_to_cpp(src),julia_to_cpp(ddepth),julia_to_cpp(ksize),julia_to_cpp(dst),julia_to_cpp(anchor),julia_to_cpp(normalize),julia_to_cpp(borderType)))
end
sqrBoxFilter(src::UMat, ddepth::Int32, ksize::Size; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSGTXP, normalize::Bool = true, borderType::Int32 = BORDER_DEFAULT) = sqrBoxFilter(src, ddepth, ksize, dst, anchor, normalize, borderType)
...
blur(src::Image, ksize::Size; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, borderType::Int32 = BORDER_DEFAULT) = blur(src, ksize, dst, anchor, borderType)

function blur(src::UMat, ksize::Size, dst::UMat, anchor::Point{Int32}, borderType::Int32)
	return cpp_to_julia(jlopencv_cv_cv_blur(julia_to_cpp(src),julia_to_cpp(ksize),julia_to_cpp(dst),julia_to_cpp(anchor),julia_to_cpp(borderType)))
end
blur(src::UMat, ksize::Size; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, borderType::Int32 = BORDER_DEFAULT) = blur(src, ksize, dst, anchor, borderType)
...
filter2D(src::Image, ddepth::Int32, kernel::Image; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, delta::Float64 = 0, borderType::Int32 = BORDER_DEFAULT) = filter2D(src, ddepth, kernel, dst, anchor, delta, borderType)

function filter2D(src::UMat, ddepth::Int32, kernel::UMat, dst::UMat, anchor::Point{Int32}, delta::Float64, borderType::Int32)
	return cpp_to_julia(jlopencv_cv_cv_filter2D(julia_to_cpp(src),julia_to_cpp(ddepth),julia_to_cpp(kernel),julia_to_cpp(dst),julia_to_cpp(anchor),julia_to_cpp(delta),julia_to_cpp(borderType)))
end
filter2D(src::UMat, ddepth::Int32, kernel::UMat; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, delta::Float64 = 0, borderType::Int32 = BORDER_DEFAULT) = filter2D(src, ddepth, kernel, dst, anchor, delta, borderType)
...
sepFilter2D(src::Image, ddepth::Int32, kernelX::Image, kernelY::Image; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, delta::Float64 = 0, borderType::Int32 = BORDER_DEFAULT) = sepFilter2D(src, ddepth, kernelX, kernelY, dst, anchor, delta, borderType)
...
sepFilter2D(src::UMat, ddepth::Int32, kernelX::UMat, kernelY::UMat; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, delta::Float64 = 0, borderType::Int32 = BORDER_DEFAULT) = sepFilter2D(src, ddepth, kernelX, kernelY, dst, anchor, delta, borderType)
...
goodFeaturesToTrack(image::Image, maxCorners::Int32, qualityLevel::Float64, minDistance::Float64; corners::Image = Mat(), mask::Image = MatOP, blockSize::Int32 = 3, useHarrisDetector::Bool = false, k::Float64 = 0.04) = goodFeaturesToTrack(image, maxCorners, qualityLevel, minDistance, corners, mask, blockSize, useHarrisDetector, k)
...
goodFeaturesToTrack(image::UMat, maxCorners::Int32, qualityLevel::Float64, minDistance::Float64; corners::UMat = UMat(), mask::UMat = UMatOP, blockSize::Int32 = 3, useHarrisDetector::Bool = false, k::Float64 = 0.04) = goodFeaturesToTrack(image, maxCorners, qualityLevel, minDistance, corners, mask, blockSize, useHarrisDetector, k)
...
erode(src::Image, kernel::Image; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, iterations::Int32 = 1, borderType::Int32 = BORDER_CONSTANT, borderValue::Scalar = morphologyDefaultBorderValueOP) = erode(src, kernel, dst, anchor, iterations, borderType, borderValue)

function erode(src::UMat, kernel::UMat, dst::UMat, anchor::Point{Int32}, iterations::Int32, borderType::Int32, borderValue::Scalar)
	return cpp_to_julia(jlopencv_cv_cv_erode(julia_to_cpp(src),julia_to_cpp(kernel),julia_to_cpp(dst),julia_to_cpp(anchor),julia_to_cpp(iterations),julia_to_cpp(borderType),julia_to_cpp(borderValue)))
end
erode(src::UMat, kernel::UMat; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, iterations::Int32 = 1, borderType::Int32 = BORDER_CONSTANT, borderValue::Scalar = morphologyDefaultBorderValueOP) = erode(src, kernel, dst, anchor, iterations, borderType, borderValue)
...
dilate(src::Image, kernel::Image; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, iterations::Int32 = 1, borderType::Int32 = BORDER_CONSTANT, borderValue::Scalar = morphologyDefaultBorderValueOP) = dilate(src, kernel, dst, anchor, iterations, borderType, borderValue)

function dilate(src::UMat, kernel::UMat, dst::UMat, anchor::Point{Int32}, iterations::Int32, borderType::Int32, borderValue::Scalar)
	return cpp_to_julia(jlopencv_cv_cv_dilate(julia_to_cpp(src),julia_to_cpp(kernel),julia_to_cpp(dst),julia_to_cpp(anchor),julia_to_cpp(iterations),julia_to_cpp(borderType),julia_to_cpp(borderValue)))
end
dilate(src::UMat, kernel::UMat; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, iterations::Int32 = 1, borderType::Int32 = BORDER_CONSTANT, borderValue::Scalar = morphologyDefaultBorderValueOP) = dilate(src, kernel, dst, anchor, iterations, borderType, borderValue)
...
morphologyEx(src::Image, op::Int32, kernel::Image; dst::Image = Mat(), anchor::Point{Int32} = PointOTXSTXP, iterations::Int32 = 1, borderType::Int32 = BORDER_CONSTANT, borderValue::Scalar = morphologyDefaultBorderValueOP) = morphologyEx(src, op, kernel, dst, anchor, iterations, borderType, borderValue)
...
morphologyEx(src::UMat, op::Int32, kernel::UMat; dst::UMat = UMat(), anchor::Point{Int32} = PointOTXSTXP, iterations::Int32 = 1, borderType::Int32 = BORDER_CONSTANT, borderValue::Scalar = morphologyDefaultBorderValueOP) = morphologyEx(src, op, kernel, dst, anchor, iterations, borderType, borderValue)
...
warpAffine(src::Image, M::Image, dsize::Size; dst::Image = Mat(), flags::Int32 = INTER_LINEAR, borderMode::Int32 = BORDER_CONSTANT, borderValue::Scalar = ScalarOP) = warpAffine(src, M, dsize, dst, flags, borderMode, borderValue)

function warpAffine(src::UMat, M::UMat, dsize::Size, dst::UMat, flags::Int32, borderMode::Int32, borderValue::Scalar)
	return cpp_to_julia(jlopencv_cv_cv_warpAffine(julia_to_cpp(src),julia_to_cpp(M),julia_to_cpp(dsize),julia_to_cpp(dst),julia_to_cpp(flags),julia_to_cpp(borderMode),julia_to_cpp(borderValue)))
end
warpAffine(src::UMat, M::UMat, dsize::Size; dst::UMat = UMat(), flags::Int32 = INTER_LINEAR, borderMode::Int32 = BORDER_CONSTANT, borderValue::Scalar = ScalarOP) = warpAffine(src, M, dsize, dst, flags, borderMode, borderValue)
...
warpPerspective(src::Image, M::Image, dsize::Size; dst::Image = Mat(), flags::Int32 = INTER_LINEAR, borderMode::Int32 = BORDER_CONSTANT, borderValue::Scalar = ScalarOP) = warpPerspective(src, M, dsize, dst, flags, borderMode, borderValue)

function warpPerspective(src::UMat, M::UMat, dsize::Size, dst::UMat, flags::Int32, borderMode::Int32, borderValue::Scalar)
	return cpp_to_julia(jlopencv_cv_cv_warpPerspective(julia_to_cpp(src),julia_to_cpp(M),julia_to_cpp(dsize),julia_to_cpp(dst),julia_to_cpp(flags),julia_to_cpp(borderMode),julia_to_cpp(borderValue)))
end
warpPerspective(src::UMat, M::UMat, dsize::Size; dst::UMat = UMat(), flags::Int32 = INTER_LINEAR, borderMode::Int32 = BORDER_CONSTANT, borderValue::Scalar = ScalarOP) = warpPerspective(src, M, dsize, dst, flags, borderMode, borderValue)
...
remap(src::Image, map1::Image, map2::Image, interpolation::Int32; dst::Image = Mat(), borderMode::Int32 = BORDER_CONSTANT, borderValue::Scalar = ScalarOP) = remap(src, map1, map2, interpolation, dst, borderMode, borderValue)

function remap(src::UMat, map1::UMat, map2::UMat, interpolation::Int32, dst::UMat, borderMode::Int32, borderValue::Scalar)
	return cpp_to_julia(jlopencv_cv_cv_remap(julia_to_cpp(src),julia_to_cpp(map1),julia_to_cpp(map2),julia_to_cpp(interpolation),julia_to_cpp(dst),julia_to_cpp(borderMode),julia_to_cpp(borderValue)))
end
remap(src::UMat, map1::UMat, map2::UMat, interpolation::Int32; dst::UMat = UMat(), borderMode::Int32 = BORDER_CONSTANT, borderValue::Scalar = ScalarOP) = remap(src, map1, map2, interpolation, dst, borderMode, borderValue)
...
pyrDown(src::Image; dst::Image = Mat(), dstsize::Size = SizeOP, borderType::Int32 = BORDER_DEFAULT) = pyrDown(src, dst, dstsize, borderType)

function pyrDown(src::UMat, dst::UMat, dstsize::Size, borderType::Int32)
	return cpp_to_julia(jlopencv_cv_cv_pyrDown(julia_to_cpp(src),julia_to_cpp(dst),julia_to_cpp(dstsize),julia_to_cpp(borderType)))
end
pyrDown(src::UMat; dst::UMat = UMat(), dstsize::Size = SizeOP, borderType::Int32 = BORDER_DEFAULT) = pyrDown(src, dst, dstsize, borderType)
...
pyrUp(src::Image; dst::Image = Mat(), dstsize::Size = SizeOP, borderType::Int32 = BORDER_DEFAULT) = pyrUp(src, dst, dstsize, borderType)

function pyrUp(src::UMat, dst::UMat, dstsize::Size, borderType::Int32)
	return cpp_to_julia(jlopencv_cv_cv_pyrUp(julia_to_cpp(src),julia_to_cpp(dst),julia_to_cpp(dstsize),julia_to_cpp(borderType)))
end
pyrUp(src::UMat; dst::UMat = UMat(), dstsize::Size = SizeOP, borderType::Int32 = BORDER_DEFAULT) = pyrUp(src, dst, dstsize, borderType)
...
createCLAHE(; clipLimit::Float64 = 40.0, tileGridSize::Size = SizeOeSGeP) = createCLAHE(clipLimit, tileGridSize)
...
EMD(signature1::Image, signature2::Image, distType::Int32; cost::Image = MatOP, lowerBound::Ptr{Float32} = PtrifloatkOP, flow::Image = MatOP) = EMD(signature1, signature2, distType, cost, lowerBound, flow)

function EMD(signature1::UMat, signature2::UMat, distType::Int32, cost::UMat, lowerBound::Ptr{Float32}, flow::UMat)
	return cpp_to_julia(jlopencv_cv_cv_wrapperEMD(julia_to_cpp(signature1),julia_to_cpp(signature2),julia_to_cpp(distType),julia_to_cpp(cost),julia_to_cpp(lowerBound),julia_to_cpp(flow)))
end
EMD(signature1::UMat, signature2::UMat, distType::Int32; cost::UMat = UMatOP, lowerBound::Ptr{Float32} = PtrifloatkOP, flow::UMat = UMatOP) = EMD(signature1, signature2, distType, cost, lowerBound, flow)
...
pyrMeanShiftFiltering(src::Image, sp::Float64, sr::Float64; dst::Image = Mat(), maxLevel::Int32 = 1, termcrit::TermCriteria = TermCriteriaOTermCriteriaggMAXRITERRTermCriteriaggEPSSbSXP) = pyrMeanShiftFiltering(src, sp, sr, dst, maxLevel, termcrit)

function pyrMeanShiftFiltering(src::UMat, sp::Float64, sr::Float64, dst::UMat, maxLevel::Int32, termcrit::TermCriteria)
	return cpp_to_julia(jlopencv_cv_cv_pyrMeanShiftFiltering(julia_to_cpp(src),julia_to_cpp(sp),julia_to_cpp(sr),julia_to_cpp(dst),julia_to_cpp(maxLevel),julia_to_cpp(termcrit)))
end
pyrMeanShiftFiltering(src::UMat, sp::Float64, sr::Float64; dst::UMat = UMat(), maxLevel::Int32 = 1, termcrit::TermCriteria = TermCriteriaOTermCriteriaggMAXRITERRTermCriteriaggEPSSbSXP) = pyrMeanShiftFiltering(src, sp, sr, dst, maxLevel, termcrit)
...
floodFill(image::Image, mask::Image, seedPoint::Point{Int32}, newVal::Scalar; loDiff::Scalar = ScalarOP, upDiff::Scalar = ScalarOP, flags::Int32 = 4) = floodFill(image, mask, seedPoint, newVal, loDiff, upDiff, flags)

function floodFill(image::UMat, mask::UMat, seedPoint::Point{Int32}, newVal::Scalar, loDiff::Scalar, upDiff::Scalar, flags::Int32)
	return cpp_to_julia(jlopencv_cv_cv_floodFill(julia_to_cpp(image),julia_to_cpp(mask),julia_to_cpp(seedPoint),julia_to_cpp(newVal),julia_to_cpp(loDiff),julia_to_cpp(upDiff),julia_to_cpp(flags)))
end
floodFill(image::UMat, mask::UMat, seedPoint::Point{Int32}, newVal::Scalar; loDiff::Scalar = ScalarOP, upDiff::Scalar = ScalarOP, flags::Int32 = 4) = floodFill(image, mask, seedPoint, newVal, loDiff, upDiff, flags)
...
fillPoly(img::Image, pts::Array{Image, 1}, color::Scalar; lineType::Int32 = LINE_8, shift::Int32 = 0, offset::Point{Int32} = PointOP) = fillPoly(img, pts, color, lineType, shift, offset)

function fillPoly(img::UMat, pts::Array{UMat, 1}, color::Scalar, lineType::Int32, shift::Int32, offset::Point{Int32})
	return cpp_to_julia(jlopencv_cv_cv_fillPoly(julia_to_cpp(img),julia_to_cpp(pts),julia_to_cpp(color),julia_to_cpp(lineType),julia_to_cpp(shift),julia_to_cpp(offset)))
end
fillPoly(img::UMat, pts::Array{UMat, 1}, color::Scalar; lineType::Int32 = LINE_8, shift::Int32 = 0, offset::Point{Int32} = PointOP) = fillPoly(img, pts, color, lineType, shift, offset)
...
drawContours(image::Image, contours::Array{Image, 1}, contourIdx::Int32, color::Scalar; thickness::Int32 = 1, lineType::Int32 = LINE_8, hierarchy::Image = MatOP, maxLevel::Int32 = INT_MAX, offset::Point{Int32} = PointOP) = drawContours(image, contours, contourIdx, color, thickness, lineType, hierarchy, maxLevel, offset)
...
drawContours(image::UMat, contours::Array{UMat, 1}, contourIdx::Int32, color::Scalar; thickness::Int32 = 1, lineType::Int32 = LINE_8, hierarchy::UMat = UMatOP, maxLevel::Int32 = INT_MAX, offset::Point{Int32} = PointOP) = drawContours(image, contours, contourIdx, color, thickness, lineType, hierarchy, maxLevel, offset)
...
findHomography(srcPoints::Image, dstPoints::Image; method::Int32 = 0, ransacReprojThreshold::Float64 = 3, mask::Image = MatOP, maxIters::Int32 = 2000, confidence::Float64 = 0.995) = findHomography(srcPoints, dstPoints, method, ransacReprojThreshold, mask, maxIters, confidence)
...
findHomography(srcPoints::UMat, dstPoints::UMat; method::Int32 = 0, ransacReprojThreshold::Float64 = 3, mask::UMat = UMatOP, maxIters::Int32 = 2000, confidence::Float64 = 0.995) = findHomography(srcPoints, dstPoints, method, ransacReprojThreshold, mask, maxIters, confidence)
...
projectPoints(objectPoints::Image, rvec::Image, tvec::Image, cameraMatrix::Image, distCoeffs::Image; imagePoints::Image = Mat(), jacobian::Image = MatOP, aspectRatio::Float64 = 0) = projectPoints(objectPoints, rvec, tvec, cameraMatrix, distCoeffs, imagePoints, jacobian, aspectRatio)
...
projectPoints(objectPoints::UMat, rvec::UMat, tvec::UMat, cameraMatrix::UMat, distCoeffs::UMat; imagePoints::UMat = UMat(), jacobian::UMat = UMatOP, aspectRatio::Float64 = 0) = projectPoints(objectPoints, rvec, tvec, cameraMatrix, distCoeffs, imagePoints, jacobian, aspectRatio)
...
estimateChessboardSharpness(image::Image, patternSize::Size, corners::Image; rise_distance::Float32 = 0.8F, vertical::Bool = false, sharpness::Image = MatOP) = estimateChessboardSharpness(image, patternSize, corners, rise_distance, vertical, sharpness)
...
estimateChessboardSharpness(image::UMat, patternSize::Size, corners::UMat; rise_distance::Float32 = 0.8F, vertical::Bool = false, sharpness::UMat = UMatOP) = estimateChessboardSharpness(image, patternSize, corners, rise_distance, vertical, sharpness)
...
function findCirclesGrid(image::Image, patternSize::Size, flags::Int32, blobDetector::Ptr{Feature2D}, parameters::CirclesGridFinderParameters, centers::Image)
...
findCirclesGrid(image::Image, patternSize::Size, flags::Int32, blobDetector::Ptr{Feature2D}, parameters::CirclesGridFinderParameters; centers::Image = Mat()) = findCirclesGrid(image, patternSize, flags, blobDetector, parameters, centers)

function findCirclesGrid(image::UMat, patternSize::Size, flags::Int32, blobDetector::Ptr{Feature2D}, parameters::CirclesGridFinderParameters, centers::UMat)
	return cpp_to_julia(jlopencv_cv_cv_findCirclesGrid(julia_to_cpp(image),julia_to_cpp(patternSize),julia_to_cpp(flags),julia_to_cpp(blobDetector),julia_to_cpp(parameters),julia_to_cpp(centers)))
end
findCirclesGrid(image::UMat, patternSize::Size, flags::Int32, blobDetector::Ptr{Feature2D}, parameters::CirclesGridFinderParameters; centers::UMat = UMat()) = findCirclesGrid(image, patternSize, flags, blobDetector, parameters, centers)

function findCirclesGrid(image::Image, patternSize::Size, centers::Image, flags::Int32, blobDetector::Ptr{Feature2D})
...
findCirclesGrid(image::Image, patternSize::Size; centers::Image = Mat(), flags::Int32 = CALIB_CB_SYMMETRIC_GRID, blobDetector::Ptr{Feature2D} = SimpleBlobDetectorggcreateOP) = findCirclesGrid(image, patternSize, centers, flags, blobDetector)

function findCirclesGrid(image::UMat, patternSize::Size, centers::UMat, flags::Int32, blobDetector::Ptr{Feature2D})
	return cpp_to_julia(jlopencv_cv_cv_findCirclesGrid(julia_to_cpp(image),julia_to_cpp(patternSize),julia_to_cpp(centers),julia_to_cpp(flags),julia_to_cpp(blobDetector)))
end
findCirclesGrid(image::UMat, patternSize::Size; centers::UMat = UMat(), flags::Int32 = CALIB_CB_SYMMETRIC_GRID, blobDetector::Ptr{Feature2D} = SimpleBlobDetectorggcreateOP) = findCirclesGrid(image, patternSize, centers, flags, blobDetector)
...
calibrateCameraExtended(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = Array{Image, 1}(), tvecs::Array{Image, 1} = Array{Image, 1}(), stdDeviationsIntrinsics::Image = Mat(), stdDeviationsExtrinsics::Image = Mat(), perViewErrors::Image = Mat(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCameraExtended(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors, flags, criteria)
...
calibrateCameraExtended(objectPoints::Array{UMat, 1}, imagePoints::Array{UMat, 1}, imageSize::Size, cameraMatrix::UMat, distCoeffs::UMat; rvecs::Array{UMat, 1} = Array{UMat, 1}(), tvecs::Array{UMat, 1} = Array{UMat, 1}(), stdDeviationsIntrinsics::UMat = UMat(), stdDeviationsExtrinsics::UMat = UMat(), perViewErrors::UMat = UMat(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCameraExtended(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors, flags, criteria)
...
calibrateCamera(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = Array{Image, 1}(), tvecs::Array{Image, 1} = Array{Image, 1}(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCamera(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, flags, criteria)
...
calibrateCamera(objectPoints::Array{UMat, 1}, imagePoints::Array{UMat, 1}, imageSize::Size, cameraMatrix::UMat, distCoeffs::UMat; rvecs::Array{UMat, 1} = Array{UMat, 1}(), tvecs::Array{UMat, 1} = Array{UMat, 1}(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCamera(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, flags, criteria)
...
calibrateCameraROExtended(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, iFixedPoint::Int32, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = Array{Image, 1}(), tvecs::Array{Image, 1} = Array{Image, 1}(), newObjPoints::Image = Mat(), stdDeviationsIntrinsics::Image = Mat(), stdDeviationsExtrinsics::Image = Mat(), stdDeviationsObjPoints::Image = Mat(), perViewErrors::Image = Mat(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCameraROExtended(objectPoints, imagePoints, imageSize, iFixedPoint, cameraMatrix, distCoeffs, rvecs, tvecs, newObjPoints, stdDeviationsIntrinsics, stdDeviationsExtrinsics, stdDeviationsObjPoints, perViewErrors, flags, criteria)
...
calibrateCameraROExtended(objectPoints::Array{UMat, 1}, imagePoints::Array{UMat, 1}, imageSize::Size, iFixedPoint::Int32, cameraMatrix::UMat, distCoeffs::UMat; rvecs::Array{UMat, 1} = Array{UMat, 1}(), tvecs::Array{UMat, 1} = Array{UMat, 1}(), newObjPoints::UMat = UMat(), stdDeviationsIntrinsics::UMat = UMat(), stdDeviationsExtrinsics::UMat = UMat(), stdDeviationsObjPoints::UMat = UMat(), perViewErrors::UMat = UMat(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCameraROExtended(objectPoints, imagePoints, imageSize, iFixedPoint, cameraMatrix, distCoeffs, rvecs, tvecs, newObjPoints, stdDeviationsIntrinsics, stdDeviationsExtrinsics, stdDeviationsObjPoints, perViewErrors, flags, criteria)
...
calibrateCameraRO(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, iFixedPoint::Int32, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = Array{Image, 1}(), tvecs::Array{Image, 1} = Array{Image, 1}(), newObjPoints::Image = Mat(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCameraRO(objectPoints, imagePoints, imageSize, iFixedPoint, cameraMatrix, distCoeffs, rvecs, tvecs, newObjPoints, flags, criteria)
...
calibrateCameraRO(objectPoints::Array{UMat, 1}, imagePoints::Array{UMat, 1}, imageSize::Size, iFixedPoint::Int32, cameraMatrix::UMat, distCoeffs::UMat; rvecs::Array{UMat, 1} = Array{UMat, 1}(), tvecs::Array{UMat, 1} = Array{UMat, 1}(), newObjPoints::UMat = UMat(), flags::Int32 = 0, criteria::TermCriteria = TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP) = calibrateCameraRO(objectPoints, imagePoints, imageSize, iFixedPoint, cameraMatrix, distCoeffs, rvecs, tvecs, newObjPoints, flags, criteria)
...
stereoCalibrateExtended(objectPoints::Array{Image, 1}, imagePoints1::Array{Image, 1}, imagePoints2::Array{Image, 1}, cameraMatrix1::Image, distCoeffs1::Image, cameraMatrix2::Image, distCoeffs2::Image, imageSize::Size, R::Image, T::Image; E::Image = Mat(), F::Image = Mat(), perViewErrors::Image = Mat(), flags::Int32 = CALIB_FIX_INTRINSIC, criteria::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGXeTcP) = stereoCalibrateExtended(objectPoints, imagePoints1, imagePoints2, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize, R, T, E, F, perViewErrors, flags, criteria)
...
stereoCalibrateExtended(objectPoints::Array{UMat, 1}, imagePoints1::Array{UMat, 1}, imagePoints2::Array{UMat, 1}, cameraMatrix1::UMat, distCoeffs1::UMat, cameraMatrix2::UMat, distCoeffs2::UMat, imageSize::Size, R::UMat, T::UMat; E::UMat = UMat(), F::UMat = UMat(), perViewErrors::UMat = UMat(), flags::Int32 = CALIB_FIX_INTRINSIC, criteria::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGXeTcP) = stereoCalibrateExtended(objectPoints, imagePoints1, imagePoints2, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize, R, T, E, F, perViewErrors, flags, criteria)
...
stereoCalibrate(objectPoints::Array{Image, 1}, imagePoints1::Array{Image, 1}, imagePoints2::Array{Image, 1}, cameraMatrix1::Image, distCoeffs1::Image, cameraMatrix2::Image, distCoeffs2::Image, imageSize::Size; R::Image = Mat(), T::Image = Mat(), E::Image = Mat(), F::Image = Mat(), flags::Int32 = CALIB_FIX_INTRINSIC, criteria::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGXeTcP) = stereoCalibrate(objectPoints, imagePoints1, imagePoints2, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize, R, T, E, F, flags, criteria)
...
stereoCalibrate(objectPoints::Array{UMat, 1}, imagePoints1::Array{UMat, 1}, imagePoints2::Array{UMat, 1}, cameraMatrix1::UMat, distCoeffs1::UMat, cameraMatrix2::UMat, distCoeffs2::UMat, imageSize::Size; R::UMat = UMat(), T::UMat = UMat(), E::UMat = UMat(), F::UMat = UMat(), flags::Int32 = CALIB_FIX_INTRINSIC, criteria::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGXeTcP) = stereoCalibrate(objectPoints, imagePoints1, imagePoints2, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize, R, T, E, F, flags, criteria)
...
findFundamentalMat(points1::Image, points2::Image; method::Int32 = FM_RANSAC, ransacReprojThreshold::Float64 = 3., confidence::Float64 = 0.99, mask::Image = MatOP) = findFundamentalMat(points1, points2, method, ransacReprojThreshold, confidence, mask)
...
findFundamentalMat(points1::UMat, points2::UMat; method::Int32 = FM_RANSAC, ransacReprojThreshold::Float64 = 3., confidence::Float64 = 0.99, mask::UMat = UMatOP) = findFundamentalMat(points1, points2, method, ransacReprojThreshold, confidence, mask)
...
findEssentialMat(points1::Image, points2::Image, cameraMatrix::Image; method::Int32 = RANSAC, prob::Float64 = 0.999, threshold::Float64 = 1.0, mask::Image = MatOP) = findEssentialMat(points1, points2, cameraMatrix, method, prob, threshold, mask)
...
findEssentialMat(points1::UMat, points2::UMat, cameraMatrix::UMat; method::Int32 = RANSAC, prob::Float64 = 0.999, threshold::Float64 = 1.0, mask::UMat = UMatOP) = findEssentialMat(points1, points2, cameraMatrix, method, prob, threshold, mask)
...
findEssentialMat(points1::Image, points2::Image; focal::Float64 = 1.0, pp::Point{Float64} = PointYdOWSGWP, method::Int32 = RANSAC, prob::Float64 = 0.999, threshold::Float64 = 1.0, mask::Image = MatOP) = findEssentialMat(points1, points2, focal, pp, method, prob, threshold, mask)
...
findEssentialMat(points1::UMat, points2::UMat; focal::Float64 = 1.0, pp::Point{Float64} = PointYdOWSGWP, method::Int32 = RANSAC, prob::Float64 = 0.999, threshold::Float64 = 1.0, mask::UMat = UMatOP) = findEssentialMat(points1, points2, focal, pp, method, prob, threshold, mask)
...
recoverPose(E::Image, points1::Image, points2::Image; R::Image = Mat(), t::Image = Mat(), focal::Float64 = 1.0, pp::Point{Float64} = PointYdOWSGWP, mask::Image = MatOP) = recoverPose(E, points1, points2, R, t, focal, pp, mask)

function recoverPose(E::UMat, points1::UMat, points2::UMat, R::UMat, t::UMat, focal::Float64, pp::Point{Float64}, mask::UMat)
	return cpp_to_julia(jlopencv_cv_cv_recoverPose(julia_to_cpp(E),julia_to_cpp(points1),julia_to_cpp(points2),julia_to_cpp(R),julia_to_cpp(t),julia_to_cpp(focal),julia_to_cpp(pp),julia_to_cpp(mask)))
end
recoverPose(E::UMat, points1::UMat, points2::UMat; R::UMat = UMat(), t::UMat = UMat(), focal::Float64 = 1.0, pp::Point{Float64} = PointYdOWSGWP, mask::UMat = UMatOP) = recoverPose(E, points1, points2, R, t, focal, pp, mask)
...
estimateAffine2D(from::Image, to::Image; inliers::Image = MatOP, method::Int32 = RANSAC, ransacReprojThreshold::Float64 = 3, maxIters::size_t = 2000, confidence::Float64 = 0.99, refineIters::size_t = 10) = estimateAffine2D(from, to, inliers, method, ransacReprojThreshold, maxIters, confidence, refineIters)
...
estimateAffine2D(from::UMat, to::UMat; inliers::UMat = UMatOP, method::Int32 = RANSAC, ransacReprojThreshold::Float64 = 3, maxIters::size_t = 2000, confidence::Float64 = 0.99, refineIters::size_t = 10) = estimateAffine2D(from, to, inliers, method, ransacReprojThreshold, maxIters, confidence, refineIters)
...
estimateAffinePartial2D(from::Image, to::Image; inliers::Image = MatOP, method::Int32 = RANSAC, ransacReprojThreshold::Float64 = 3, maxIters::size_t = 2000, confidence::Float64 = 0.99, refineIters::size_t = 10) = estimateAffinePartial2D(from, to, inliers, method, ransacReprojThreshold, maxIters, confidence, refineIters)
...
estimateAffinePartial2D(from::UMat, to::UMat; inliers::UMat = UMatOP, method::Int32 = RANSAC, ransacReprojThreshold::Float64 = 3, maxIters::size_t = 2000, confidence::Float64 = 0.99, refineIters::size_t = 10) = estimateAffinePartial2D(from, to, inliers, method, ransacReprojThreshold, maxIters, confidence, refineIters)
...
BRISK_create(radiusList::Array{Float32, 1}, numberList::Array{Int32, 1}; dMax::Float32 = 5.85f, dMin::Float32 = 8.2f, indexChange::Array{Int32, 1} = stdggvectoriintkOP) = BRISK_create(radiusList, numberList, dMax, dMin, indexChange)
...
BRISK_create(thresh::Int32, octaves::Int32, radiusList::Array{Float32, 1}, numberList::Array{Int32, 1}; dMax::Float32 = 5.85f, dMin::Float32 = 8.2f, indexChange::Array{Int32, 1} = stdggvectoriintkOP) = BRISK_create(thresh, octaves, radiusList, numberList, dMax, dMin, indexChange)
...
ORB_create(; nfeatures::Int32 = 500, scaleFactor::Float32 = 1.2f, nlevels::Int32 = 8, edgeThreshold::Int32 = 31, firstLevel::Int32 = 0, WTA_K::Int32 = 2, scoreType::ORB_ScoreType = ORBggHARRISRSCORE, patchSize::Int32 = 31, fastThreshold::Int32 = 20) = ORB_create(nfeatures, scaleFactor, nlevels, edgeThreshold, firstLevel, WTA_K, scoreType, patchSize, fastThreshold)
...
FastFeatureDetector_create(; threshold::Int32 = 10, nonmaxSuppression::Bool = true, type::FastFeatureDetector_DetectorType = FastFeatureDetectorggTYPERfRXc) = FastFeatureDetector_create(threshold, nonmaxSuppression, type)
...
AgastFeatureDetector_create(; threshold::Int32 = 10, nonmaxSuppression::Bool = true, type::AgastFeatureDetector_DetectorType = AgastFeatureDetectorggOASTRfRXc) = AgastFeatureDetector_create(threshold, nonmaxSuppression, type)
...
KAZE_create(; extended::Bool = false, upright::Bool = false, threshold::Float32 = 0.001f, nOctaves::Int32 = 4, nOctaveLayers::Int32 = 4, diffusivity::KAZE_DiffusivityType = KAZEggDIFFRPMRGY) = KAZE_create(extended, upright, threshold, nOctaves, nOctaveLayers, diffusivity)
...
AKAZE_create(; descriptor_type::AKAZE_DescriptorType = AKAZEggDESCRIPTORRMLDB, descriptor_size::Int32 = 0, descriptor_channels::Int32 = 3, threshold::Float32 = 0.001f, nOctaves::Int32 = 4, nOctaveLayers::Int32 = 4, diffusivity::KAZE_DiffusivityType = KAZEggDIFFRPMRGY) = AKAZE_create(descriptor_type, descriptor_size, descriptor_channels, threshold, nOctaves, nOctaveLayers, diffusivity)
...
calcOpticalFlowPyrLK(prevImg::Image, nextImg::Image, prevPts::Image, nextPts::Image; status::Image = Mat(), err::Image = Mat(), winSize::Size = SizeOYXSYXP, maxLevel::Int32 = 3, criteria::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGWUWXP, flags::Int32 = 0, minEigThreshold::Float64 = 1e-4) = calcOpticalFlowPyrLK(prevImg, nextImg, prevPts, nextPts, status, err, winSize, maxLevel, criteria, flags, minEigThreshold)
...
calcOpticalFlowPyrLK(prevImg::UMat, nextImg::UMat, prevPts::UMat, nextPts::UMat; status::UMat = UMat(), err::UMat = UMat(), winSize::Size = SizeOYXSYXP, maxLevel::Int32 = 3, criteria::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGWUWXP, flags::Int32 = 0, minEigThreshold::Float64 = 1e-4) = calcOpticalFlowPyrLK(prevImg, nextImg, prevPts, nextPts, status, err, winSize, maxLevel, criteria, flags, minEigThreshold)
...
SparsePyrLKOpticalFlow_create(; winSize::Size = SizeOYXSGYXP, maxLevel::Int32 = 3, crit::TermCriteria = TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGWUWXP, flags::Int32 = 0, minEigThreshold::Float64 = 1e-4) = SparsePyrLKOpticalFlow_create(winSize, maxLevel, crit, flags, minEigThreshold)
...
addText(img::Image, text::String, org::Point{Int32}, nameFont::String; pointSize::Int32 = -1, color::Scalar = ScalarggallOWP, weight::Int32 = QT_FONT_NORMAL, style::Int32 = QT_STYLE_NORMAL, spacing::Int32 = 0) = addText(img, text, org, nameFont, pointSize, color, weight, style, spacing)
