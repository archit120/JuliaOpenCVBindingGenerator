...
detect(cobj::cv_Ptr{T}, image::Image; positions::Image = (CxxMat()), votes::Image = (MatOP)) where {T <: GeneralizedHough} = detect(cobj, image, positions, votes)
...
apply(cobj::cv_Ptr{T}, src::Image; dst::Image = (CxxMat())) where {T <: CLAHE} = apply(cobj, src, dst)
...
detect(cobj::cv_Ptr{T}, _image::Image; _lines::Image = (CxxMat()), width::Image = (MatOP), prec::Image = (MatOP), nfa::Image = (MatOP)) where {T <: LineSegmentDetector} = detect(cobj, _image, _lines, width, prec, nfa)
...
compute(cobj::cv_Ptr{T}, left::Image, right::Image; disparity::Image = (CxxMat())) where {T <: StereoMatcher} = compute(cobj, left, right, disparity)
...
compute(cobj::cv_Ptr{T}, image::Image, keypoints::Array{KeyPoint, 1}; descriptors::Image = (CxxMat())) where {T <: Feature2D} = compute(cobj, image, keypoints, descriptors)
...
detectAndCompute(cobj::cv_Ptr{T}, image::Image, mask::Image; descriptors::Image = (CxxMat()), useProvidedKeypoints::Bool = (false)) where {T <: Feature2D} = detectAndCompute(cobj, image, mask, descriptors, useProvidedKeypoints)
...
compute2(cobj::BOWImgDescriptorExtractor, image::Image, keypoints::Array{KeyPoint, 1}; imgDescriptor::Image = (CxxMat())) = compute2(cobj, image, keypoints, imgDescriptor)
...
function BOWImgDescriptorExtractor(dextractor::cv_Ptr{Feature2D}, dmatcher::cv_Ptr{Feature2D})
...
calc(cobj::cv_Ptr{T}, prevImg::Image, nextImg::Image, prevPts::Image, nextPts::Image; status::Image = (CxxMat()), err::Image = (cvggMatOP)) where {T <: SparseOpticalFlow} = calc(cobj, prevImg, nextImg, prevPts, nextPts, status, err)
...
apply(cobj::cv_Ptr{T}, image::Image; fgmask::Image = (CxxMat()), learningRate::Float64 = Float64(-1)) where {T <: BackgroundSubtractor} = apply(cobj, image, fgmask, learningRate)
...
getBackgroundImage(cobj::cv_Ptr{T}; backgroundImage::Image = (CxxMat())) where {T <: BackgroundSubtractor} = getBackgroundImage(cobj, backgroundImage)
...
apply(cobj::cv_Ptr{T}, image::Image; fgmask::Image = (CxxMat()), learningRate::Float64 = Float64(-1)) where {T <: BackgroundSubtractorMOG2} = apply(cobj, image, fgmask, learningRate)
...
detect(cobj::QRCodeDetector, img::Image; points::Image = (CxxMat())) = detect(cobj, img, points)
...
detectMulti(cobj::QRCodeDetector, img::Image; points::Image = (CxxMat())) = detectMulti(cobj, img, points)
...
retrieve(cobj::VideoCapture; image::Image = (CxxMat()), flag::Int32 = Int32(0)) = retrieve(cobj, image, flag)
...
read(cobj::VideoCapture; image::Image = (CxxMat())) = read(cobj, image)
...
getDerivKernels(dx::Int32, dy::Int32, ksize::Int32; kx::Image = (CxxMat()), ky::Image = (CxxMat()), normalize::Bool = (false), ktype::Int32 = Int32(CV_32F)) = getDerivKernels(dx, dy, ksize, kx, ky, normalize, ktype)
...
medianBlur(src::Image, ksize::Int32; dst::Image = (CxxMat())) = medianBlur(src, ksize, dst)
...
GaussianBlur(src::Image, ksize::Size, sigmaX::Float64; dst::Image = (CxxMat()), sigmaY::Float64 = Float64(0), borderType::Int32 = Int32(BORDER_DEFAULT)) = GaussianBlur(src, ksize, sigmaX, dst, sigmaY, borderType)
...
bilateralFilter(src::Image, d::Int32, sigmaColor::Float64, sigmaSpace::Float64; dst::Image = (CxxMat()), borderType::Int32 = Int32(BORDER_DEFAULT)) = bilateralFilter(src, d, sigmaColor, sigmaSpace, dst, borderType)
...
boxFilter(src::Image, ddepth::Int32, ksize::Size; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), normalize::Bool = (true), borderType::Int32 = Int32(BORDER_DEFAULT)) = boxFilter(src, ddepth, ksize, dst, anchor, normalize, borderType)
...
sqrBoxFilter(src::Image, ddepth::Int32, ksize::Size; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSGTXP), normalize::Bool = (true), borderType::Int32 = Int32(BORDER_DEFAULT)) = sqrBoxFilter(src, ddepth, ksize, dst, anchor, normalize, borderType)
...
blur(src::Image, ksize::Size; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), borderType::Int32 = Int32(BORDER_DEFAULT)) = blur(src, ksize, dst, anchor, borderType)
...
filter2D(src::Image, ddepth::Int32, kernel::Image; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), delta::Float64 = Float64(0), borderType::Int32 = Int32(BORDER_DEFAULT)) = filter2D(src, ddepth, kernel, dst, anchor, delta, borderType)
...
sepFilter2D(src::Image, ddepth::Int32, kernelX::Image, kernelY::Image; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), delta::Float64 = Float64(0), borderType::Int32 = Int32(BORDER_DEFAULT)) = sepFilter2D(src, ddepth, kernelX, kernelY, dst, anchor, delta, borderType)
...
Sobel(src::Image, ddepth::Int32, dx::Int32, dy::Int32; dst::Image = (CxxMat()), ksize::Int32 = Int32(3), scale::Float64 = Float64(1), delta::Float64 = Float64(0), borderType::Int32 = Int32(BORDER_DEFAULT)) = Sobel(src, ddepth, dx, dy, dst, ksize, scale, delta, borderType)
...
spatialGradient(src::Image; dx::Image = (CxxMat()), dy::Image = (CxxMat()), ksize::Int32 = Int32(3), borderType::Int32 = Int32(BORDER_DEFAULT)) = spatialGradient(src, dx, dy, ksize, borderType)
...
Scharr(src::Image, ddepth::Int32, dx::Int32, dy::Int32; dst::Image = (CxxMat()), scale::Float64 = Float64(1), delta::Float64 = Float64(0), borderType::Int32 = Int32(BORDER_DEFAULT)) = Scharr(src, ddepth, dx, dy, dst, scale, delta, borderType)
...
Laplacian(src::Image, ddepth::Int32; dst::Image = (CxxMat()), ksize::Int32 = Int32(1), scale::Float64 = Float64(1), delta::Float64 = Float64(0), borderType::Int32 = Int32(BORDER_DEFAULT)) = Laplacian(src, ddepth, dst, ksize, scale, delta, borderType)
...
Canny(image::Image, threshold1::Float64, threshold2::Float64; edges::Image = (CxxMat()), apertureSize::Int32 = Int32(3), L2gradient::Bool = (false)) = Canny(image, threshold1, threshold2, edges, apertureSize, L2gradient)
...
Canny(dx::Image, dy::Image, threshold1::Float64, threshold2::Float64; edges::Image = (CxxMat()), L2gradient::Bool = (false)) = Canny(dx, dy, threshold1, threshold2, edges, L2gradient)
...
cornerMinEigenVal(src::Image, blockSize::Int32; dst::Image = (CxxMat()), ksize::Int32 = Int32(3), borderType::Int32 = Int32(BORDER_DEFAULT)) = cornerMinEigenVal(src, blockSize, dst, ksize, borderType)
...
cornerHarris(src::Image, blockSize::Int32, ksize::Int32, k::Float64; dst::Image = (CxxMat()), borderType::Int32 = Int32(BORDER_DEFAULT)) = cornerHarris(src, blockSize, ksize, k, dst, borderType)
...
cornerEigenValsAndVecs(src::Image, blockSize::Int32, ksize::Int32; dst::Image = (CxxMat()), borderType::Int32 = Int32(BORDER_DEFAULT)) = cornerEigenValsAndVecs(src, blockSize, ksize, dst, borderType)
...
preCornerDetect(src::Image, ksize::Int32; dst::Image = (CxxMat()), borderType::Int32 = Int32(BORDER_DEFAULT)) = preCornerDetect(src, ksize, dst, borderType)
...
goodFeaturesToTrack(image::Image, maxCorners::Int32, qualityLevel::Float64, minDistance::Float64; corners::Image = (CxxMat()), mask::Image = (MatOP), blockSize::Int32 = Int32(3), useHarrisDetector::Bool = (false), k::Float64 = Float64(0.04)) = goodFeaturesToTrack(image, maxCorners, qualityLevel, minDistance, corners, mask, blockSize, useHarrisDetector, k)
...
goodFeaturesToTrack(image::Image, maxCorners::Int32, qualityLevel::Float64, minDistance::Float64, mask::Image, blockSize::Int32, gradientSize::Int32; corners::Image = (CxxMat()), useHarrisDetector::Bool = (false), k::Float64 = Float64(0.04)) = goodFeaturesToTrack(image, maxCorners, qualityLevel, minDistance, mask, blockSize, gradientSize, corners, useHarrisDetector, k)
...
HoughLines(image::Image, rho::Float64, theta::Float64, threshold::Int32; lines::Image = (CxxMat()), srn::Float64 = Float64(0), stn::Float64 = Float64(0), min_theta::Float64 = Float64(0), max_theta::Float64 = Float64(CV_PI)) = HoughLines(image, rho, theta, threshold, lines, srn, stn, min_theta, max_theta)
...
HoughLinesP(image::Image, rho::Float64, theta::Float64, threshold::Int32; lines::Image = (CxxMat()), minLineLength::Float64 = Float64(0), maxLineGap::Float64 = Float64(0)) = HoughLinesP(image, rho, theta, threshold, lines, minLineLength, maxLineGap)
...
HoughLinesPointSet(_point::Image, lines_max::Int32, threshold::Int32, min_rho::Float64, max_rho::Float64, rho_step::Float64, min_theta::Float64, max_theta::Float64, theta_step::Float64; _lines::Image = (CxxMat())) = HoughLinesPointSet(_point, lines_max, threshold, min_rho, max_rho, rho_step, min_theta, max_theta, theta_step, _lines)
...
HoughCircles(image::Image, method::Int32, dp::Float64, minDist::Float64; circles::Image = (CxxMat()), param1::Float64 = Float64(100), param2::Float64 = Float64(100), minRadius::Int32 = Int32(0), maxRadius::Int32 = Int32(0)) = HoughCircles(image, method, dp, minDist, circles, param1, param2, minRadius, maxRadius)
...
erode(src::Image, kernel::Image; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), iterations::Int32 = Int32(1), borderType::Int32 = Int32(BORDER_CONSTANT), borderValue::Scalar = (morphologyDefaultBorderValueOP)) = erode(src, kernel, dst, anchor, iterations, borderType, borderValue)
...
dilate(src::Image, kernel::Image; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), iterations::Int32 = Int32(1), borderType::Int32 = Int32(BORDER_CONSTANT), borderValue::Scalar = (morphologyDefaultBorderValueOP)) = dilate(src, kernel, dst, anchor, iterations, borderType, borderValue)
...
morphologyEx(src::Image, op::Int32, kernel::Image; dst::Image = (CxxMat()), anchor::Point{Int32} = (PointOTXSTXP), iterations::Int32 = Int32(1), borderType::Int32 = Int32(BORDER_CONSTANT), borderValue::Scalar = (morphologyDefaultBorderValueOP)) = morphologyEx(src, op, kernel, dst, anchor, iterations, borderType, borderValue)
...
resize(src::Image, dsize::Size; dst::Image = (CxxMat()), fx::Float64 = Float64(0), fy::Float64 = Float64(0), interpolation::Int32 = Int32(INTER_LINEAR)) = resize(src, dsize, dst, fx, fy, interpolation)
...
warpAffine(src::Image, M::Image, dsize::Size; dst::Image = (CxxMat()), flags::Int32 = Int32(INTER_LINEAR), borderMode::Int32 = Int32(BORDER_CONSTANT), borderValue::Scalar = (ScalarOP)) = warpAffine(src, M, dsize, dst, flags, borderMode, borderValue)
...
warpPerspective(src::Image, M::Image, dsize::Size; dst::Image = (CxxMat()), flags::Int32 = Int32(INTER_LINEAR), borderMode::Int32 = Int32(BORDER_CONSTANT), borderValue::Scalar = (ScalarOP)) = warpPerspective(src, M, dsize, dst, flags, borderMode, borderValue)
...
remap(src::Image, map1::Image, map2::Image, interpolation::Int32; dst::Image = (CxxMat()), borderMode::Int32 = Int32(BORDER_CONSTANT), borderValue::Scalar = (ScalarOP)) = remap(src, map1, map2, interpolation, dst, borderMode, borderValue)
...
convertMaps(map1::Image, map2::Image, dstmap1type::Int32; dstmap1::Image = (CxxMat()), dstmap2::Image = (CxxMat()), nninterpolation::Bool = (false)) = convertMaps(map1, map2, dstmap1type, dstmap1, dstmap2, nninterpolation)
...
invertAffineTransform(M::Image; iM::Image = (CxxMat())) = invertAffineTransform(M, iM)
...
getRectSubPix(image::Image, patchSize::Size, center::Point{Float32}; patch::Image = (CxxMat()), patchType::Int32 = Int32(-1)) = getRectSubPix(image, patchSize, center, patch, patchType)
...
logPolar(src::Image, center::Point{Float32}, M::Float64, flags::Int32; dst::Image = (CxxMat())) = logPolar(src, center, M, flags, dst)
...
linearPolar(src::Image, center::Point{Float32}, maxRadius::Float64, flags::Int32; dst::Image = (CxxMat())) = linearPolar(src, center, maxRadius, flags, dst)
...
warpPolar(src::Image, dsize::Size, center::Point{Float32}, maxRadius::Float64, flags::Int32; dst::Image = (CxxMat())) = warpPolar(src, dsize, center, maxRadius, flags, dst)
...
integral(src::Image; sum::Image = (CxxMat()), sdepth::Int32 = Int32(-1)) = integral(src, sum, sdepth)
...
integral2(src::Image; sum::Image = (CxxMat()), sqsum::Image = (CxxMat()), sdepth::Int32 = Int32(-1), sqdepth::Int32 = Int32(-1)) = integral2(src, sum, sqsum, sdepth, sqdepth)
...
integral3(src::Image; sum::Image = (CxxMat()), sqsum::Image = (CxxMat()), tilted::Image = (CxxMat()), sdepth::Int32 = Int32(-1), sqdepth::Int32 = Int32(-1)) = integral3(src, sum, sqsum, tilted, sdepth, sqdepth)
...
createHanningWindow(winSize::Size, type::Int32; dst::Image = (CxxMat())) = createHanningWindow(winSize, type, dst)
...
threshold(src::Image, thresh::Float64, maxval::Float64, type::Int32; dst::Image = (CxxMat())) = threshold(src, thresh, maxval, type, dst)
...
adaptiveThreshold(src::Image, maxValue::Float64, adaptiveMethod::Int32, thresholdType::Int32, blockSize::Int32, C::Float64; dst::Image = (CxxMat())) = adaptiveThreshold(src, maxValue, adaptiveMethod, thresholdType, blockSize, C, dst)
...
pyrDown(src::Image; dst::Image = (CxxMat()), dstsize::Size = (SizeOP), borderType::Int32 = Int32(BORDER_DEFAULT)) = pyrDown(src, dst, dstsize, borderType)
...
pyrUp(src::Image; dst::Image = (CxxMat()), dstsize::Size = (SizeOP), borderType::Int32 = Int32(BORDER_DEFAULT)) = pyrUp(src, dst, dstsize, borderType)
...
calcHist(images::Array{Image, 1}, channels::Array{Int32, 1}, mask::Image, histSize::Array{Int32, 1}, ranges::Array{Float32, 1}; hist::Image = (CxxMat()), accumulate::Bool = (false)) = calcHist(images, channels, mask, histSize, ranges, hist, accumulate)
...
calcBackProject(images::Array{Image, 1}, channels::Array{Int32, 1}, hist::Image, ranges::Array{Float32, 1}, scale::Float64; dst::Image = (CxxMat())) = calcBackProject(images, channels, hist, ranges, scale, dst)
...
equalizeHist(src::Image; dst::Image = (CxxMat())) = equalizeHist(src, dst)
...
pyrMeanShiftFiltering(src::Image, sp::Float64, sr::Float64; dst::Image = (CxxMat()), maxLevel::Int32 = Int32(1), termcrit::TermCriteria = (TermCriteriaOTermCriteriaggMAXRITERRTermCriteriaggEPSSbSXP)) = pyrMeanShiftFiltering(src, sp, sr, dst, maxLevel, termcrit)
...
distanceTransformWithLabels(src::Image, distanceType::Int32, maskSize::Int32; dst::Image = (CxxMat()), labels::Image = (CxxMat()), labelType::Int32 = Int32(DIST_LABEL_CCOMP)) = distanceTransformWithLabels(src, distanceType, maskSize, dst, labels, labelType)
...
distanceTransform(src::Image, distanceType::Int32, maskSize::Int32; dst::Image = (CxxMat()), dstType::Int32 = Int32(CV_32F)) = distanceTransform(src, distanceType, maskSize, dst, dstType)
...
cvtColor(src::Image, code::Int32; dst::Image = (CxxMat()), dstCn::Int32 = Int32(0)) = cvtColor(src, code, dst, dstCn)
...
cvtColorTwoPlane(src1::Image, src2::Image, code::Int32; dst::Image = (CxxMat())) = cvtColorTwoPlane(src1, src2, code, dst)
...
demosaicing(src::Image, code::Int32; dst::Image = (CxxMat()), dstCn::Int32 = Int32(0)) = demosaicing(src, code, dst, dstCn)
...
HuMoments(m::Moments; hu::Image = (CxxMat())) = HuMoments(m, hu)
...
matchTemplate(image::Image, templ::Image, method::Int32; result::Image = (CxxMat()), mask::Image = (MatOP)) = matchTemplate(image, templ, method, result, mask)
...
connectedComponentsWithAlgorithm(image::Image, connectivity::Int32, ltype::Int32, ccltype::Int32; labels::Image = (CxxMat())) = connectedComponentsWithAlgorithm(image, connectivity, ltype, ccltype, labels)
...
connectedComponents(image::Image; labels::Image = (CxxMat()), connectivity::Int32 = Int32(8), ltype::Int32 = Int32(CV_32S)) = connectedComponents(image, labels, connectivity, ltype)
...
connectedComponentsWithStatsWithAlgorithm(image::Image, connectivity::Int32, ltype::Int32, ccltype::Int32; labels::Image = (CxxMat()), stats::Image = (CxxMat()), centroids::Image = (CxxMat())) = connectedComponentsWithStatsWithAlgorithm(image, connectivity, ltype, ccltype, labels, stats, centroids)
...
connectedComponentsWithStats(image::Image; labels::Image = (CxxMat()), stats::Image = (CxxMat()), centroids::Image = (CxxMat()), connectivity::Int32 = Int32(8), ltype::Int32 = Int32(CV_32S)) = connectedComponentsWithStats(image, labels, stats, centroids, connectivity, ltype)
...
findContours(image::Image, mode::Int32, method::Int32; contours::Array{Image, 1} = (Array{Image, 1}()), hierarchy::Image = (CxxMat()), offset::Point{Int32} = (PointOP)) = findContours(image, mode, method, contours, hierarchy, offset)
...
approxPolyDP(curve::Image, epsilon::Float64, closed::Bool; approxCurve::Image = (CxxMat())) = approxPolyDP(curve, epsilon, closed, approxCurve)
...
boxPoints(box::RotatedRect; points::Image = (CxxMat())) = boxPoints(box, points)
...
minEnclosingTriangle(points::Image; triangle::Image = (CxxMat())) = minEnclosingTriangle(points, triangle)
...
convexHull(points::Image; hull::Image = (CxxMat()), clockwise::Bool = (false), returnPoints::Bool = (true)) = convexHull(points, hull, clockwise, returnPoints)
...
convexityDefects(contour::Image, convexhull::Image; convexityDefects::Image = (CxxMat())) = convexityDefects(contour, convexhull, convexityDefects)
...
intersectConvexConvex(_p1::Image, _p2::Image; _p12::Image = (CxxMat()), handleNested::Bool = (true)) = intersectConvexConvex(_p1, _p2, _p12, handleNested)
...
fitLine(points::Image, distType::Int32, param::Float64, reps::Float64, aeps::Float64; line::Image = (CxxMat())) = fitLine(points, distType, param, reps, aeps, line)
...
rotatedRectangleIntersection(rect1::RotatedRect, rect2::RotatedRect; intersectingRegion::Image = (CxxMat())) = rotatedRectangleIntersection(rect1, rect2, intersectingRegion)
...
applyColorMap(src::Image, colormap::Int32; dst::Image = (CxxMat())) = applyColorMap(src, colormap, dst)
...
applyColorMap(src::Image, userColor::Image; dst::Image = (CxxMat())) = applyColorMap(src, userColor, dst)
...
Rodrigues(src::Image; dst::Image = (CxxMat()), jacobian::Image = (MatOP)) = Rodrigues(src, dst, jacobian)
...
RQDecomp3x3(src::Image; mtxR::Image = (CxxMat()), mtxQ::Image = (CxxMat()), Qx::Image = (MatOP), Qy::Image = (MatOP), Qz::Image = (MatOP)) = RQDecomp3x3(src, mtxR, mtxQ, Qx, Qy, Qz)
...
decomposeProjectionMatrix(projMatrix::Image; cameraMatrix::Image = (CxxMat()), rotMatrix::Image = (CxxMat()), transVect::Image = (CxxMat()), rotMatrixX::Image = (MatOP), rotMatrixY::Image = (MatOP), rotMatrixZ::Image = (MatOP), eulerAngles::Image = (MatOP)) = decomposeProjectionMatrix(projMatrix, cameraMatrix, rotMatrix, transVect, rotMatrixX, rotMatrixY, rotMatrixZ, eulerAngles)
...
matMulDeriv(A::Image, B::Image; dABdA::Image = (CxxMat()), dABdB::Image = (CxxMat())) = matMulDeriv(A, B, dABdA, dABdB)
...
projectPoints(objectPoints::Image, rvec::Image, tvec::Image, cameraMatrix::Image, distCoeffs::Image; imagePoints::Image = (CxxMat()), jacobian::Image = (MatOP), aspectRatio::Float64 = Float64(0)) = projectPoints(objectPoints, rvec, tvec, cameraMatrix, distCoeffs, imagePoints, jacobian, aspectRatio)
...
solvePnP(objectPoints::Image, imagePoints::Image, cameraMatrix::Image, distCoeffs::Image; rvec::Image = (CxxMat()), tvec::Image = (CxxMat()), useExtrinsicGuess::Bool = (false), flags::Int32 = Int32(SOLVEPNP_ITERATIVE)) = solvePnP(objectPoints, imagePoints, cameraMatrix, distCoeffs, rvec, tvec, useExtrinsicGuess, flags)
...
findChessboardCorners(image::Image, patternSize::Size; corners::Image = (CxxMat()), flags::Int32 = Int32(CALIB_CB_ADAPTIVE_THRESH + CALIB_CB_NORMALIZE_IMAGE)) = findChessboardCorners(image, patternSize, corners, flags)
...
findChessboardCornersSBWithMeta(image::Image, patternSize::Size, flags::Int32; corners::Image = (CxxMat()), meta::Image = (CxxMat())) = findChessboardCornersSBWithMeta(image, patternSize, flags, corners, meta)
...
findChessboardCornersSB(image::Image, patternSize::Size; corners::Image = (CxxMat()), flags::Int32 = Int32(0)) = findChessboardCornersSB(image, patternSize, corners, flags)
...
findCirclesGrid(image::Image, patternSize::Size, flags::Int32, blobDetector::cv_Ptr{Feature2D}, parameters::CirclesGridFinderParameters; centers::Image = (CxxMat())) = findCirclesGrid(image, patternSize, flags, blobDetector, parameters, centers)
...
findCirclesGrid(image::Image, patternSize::Size; centers::Image = (CxxMat()), flags::Int32 = Int32(CALIB_CB_SYMMETRIC_GRID), blobDetector::cv_Ptr{Feature2D} = (SimpleBlobDetectorggcreateOP)) = findCirclesGrid(image, patternSize, centers, flags, blobDetector)
...
calibrateCameraExtended(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = (Array{Image, 1}()), tvecs::Array{Image, 1} = (Array{Image, 1}()), stdDeviationsIntrinsics::Image = (CxxMat()), stdDeviationsExtrinsics::Image = (CxxMat()), perViewErrors::Image = (CxxMat()), flags::Int32 = Int32(0), criteria::TermCriteria = (TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP)) = calibrateCameraExtended(objectPoints, imagePoints, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors, flags, criteria)
...
calibrateCameraROExtended(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, iFixedPoint::Int32, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = (Array{Image, 1}()), tvecs::Array{Image, 1} = (Array{Image, 1}()), newObjPoints::Image = (CxxMat()), stdDeviationsIntrinsics::Image = (CxxMat()), stdDeviationsExtrinsics::Image = (CxxMat()), stdDeviationsObjPoints::Image = (CxxMat()), perViewErrors::Image = (CxxMat()), flags::Int32 = Int32(0), criteria::TermCriteria = (TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP)) = calibrateCameraROExtended(objectPoints, imagePoints, imageSize, iFixedPoint, cameraMatrix, distCoeffs, rvecs, tvecs, newObjPoints, stdDeviationsIntrinsics, stdDeviationsExtrinsics, stdDeviationsObjPoints, perViewErrors, flags, criteria)
...
calibrateCameraRO(objectPoints::Array{Image, 1}, imagePoints::Array{Image, 1}, imageSize::Size, iFixedPoint::Int32, cameraMatrix::Image, distCoeffs::Image; rvecs::Array{Image, 1} = (Array{Image, 1}()), tvecs::Array{Image, 1} = (Array{Image, 1}()), newObjPoints::Image = (CxxMat()), flags::Int32 = Int32(0), criteria::TermCriteria = (TermCriteriaOGTermCriteriaggCOUNTGRGTermCriteriaggEPSSGZWSGDBLREPSILONP)) = calibrateCameraRO(objectPoints, imagePoints, imageSize, iFixedPoint, cameraMatrix, distCoeffs, rvecs, tvecs, newObjPoints, flags, criteria)
...
stereoCalibrateExtended(objectPoints::Array{Image, 1}, imagePoints1::Array{Image, 1}, imagePoints2::Array{Image, 1}, cameraMatrix1::Image, distCoeffs1::Image, cameraMatrix2::Image, distCoeffs2::Image, imageSize::Size, R::Image, T::Image; E::Image = (CxxMat()), F::Image = (CxxMat()), perViewErrors::Image = (CxxMat()), flags::Int32 = Int32(CALIB_FIX_INTRINSIC), criteria::TermCriteria = (TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGXeTcP)) = stereoCalibrateExtended(objectPoints, imagePoints1, imagePoints2, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize, R, T, E, F, perViewErrors, flags, criteria)
...
stereoCalibrate(objectPoints::Array{Image, 1}, imagePoints1::Array{Image, 1}, imagePoints2::Array{Image, 1}, cameraMatrix1::Image, distCoeffs1::Image, cameraMatrix2::Image, distCoeffs2::Image, imageSize::Size; R::Image = (CxxMat()), T::Image = (CxxMat()), E::Image = (CxxMat()), F::Image = (CxxMat()), flags::Int32 = Int32(CALIB_FIX_INTRINSIC), criteria::TermCriteria = (TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGXeTcP)) = stereoCalibrate(objectPoints, imagePoints1, imagePoints2, cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, imageSize, R, T, E, F, flags, criteria)
...
stereoRectifyUncalibrated(points1::Image, points2::Image, F::Image, imgSize::Size; H1::Image = (CxxMat()), H2::Image = (CxxMat()), threshold::Float64 = Float64(5)) = stereoRectifyUncalibrated(points1, points2, F, imgSize, H1, H2, threshold)
...
rectify3Collinear(cameraMatrix1::Image, distCoeffs1::Image, cameraMatrix2::Image, distCoeffs2::Image, cameraMatrix3::Image, distCoeffs3::Image, imgpt1::Array{Image, 1}, imgpt3::Array{Image, 1}, imageSize::Size, R12::Image, T12::Image, R13::Image, T13::Image, alpha::Float64, newImgSize::Size, flags::Int32; R1::Image = (CxxMat()), R2::Image = (CxxMat()), R3::Image = (CxxMat()), P1::Image = (CxxMat()), P2::Image = (CxxMat()), P3::Image = (CxxMat()), Q::Image = (CxxMat())) = rectify3Collinear(cameraMatrix1, distCoeffs1, cameraMatrix2, distCoeffs2, cameraMatrix3, distCoeffs3, imgpt1, imgpt3, imageSize, R12, T12, R13, T13, alpha, newImgSize, flags, R1, R2, R3, P1, P2, P3, Q)
...
calibrateHandEye(R_gripper2base::Array{Image, 1}, t_gripper2base::Array{Image, 1}, R_target2cam::Array{Image, 1}, t_target2cam::Array{Image, 1}; R_cam2gripper::Image = (CxxMat()), t_cam2gripper::Image = (CxxMat()), method::HandEyeCalibrationMethod = (CALIB_HAND_EYE_TSAI)) = calibrateHandEye(R_gripper2base, t_gripper2base, R_target2cam, t_target2cam, R_cam2gripper, t_cam2gripper, method)
...
convertPointsToHomogeneous(src::Image; dst::Image = (CxxMat())) = convertPointsToHomogeneous(src, dst)
...
convertPointsFromHomogeneous(src::Image; dst::Image = (CxxMat())) = convertPointsFromHomogeneous(src, dst)
...
decomposeEssentialMat(E::Image; R1::Image = (CxxMat()), R2::Image = (CxxMat()), t::Image = (CxxMat())) = decomposeEssentialMat(E, R1, R2, t)
...
recoverPose(E::Image, points1::Image, points2::Image, cameraMatrix::Image; R::Image = (CxxMat()), t::Image = (CxxMat()), mask::Image = (MatOP)) = recoverPose(E, points1, points2, cameraMatrix, R, t, mask)
...
recoverPose(E::Image, points1::Image, points2::Image; R::Image = (CxxMat()), t::Image = (CxxMat()), focal::Float64 = Float64(1.0), pp::Point{Float64} = (PointYdOWSGWP), mask::Image = (MatOP)) = recoverPose(E, points1, points2, R, t, focal, pp, mask)
...
recoverPose(E::Image, points1::Image, points2::Image, cameraMatrix::Image, distanceThresh::Float64; R::Image = (CxxMat()), t::Image = (CxxMat()), mask::Image = (MatOP), triangulatedPoints::Image = (MatOP)) = recoverPose(E, points1, points2, cameraMatrix, distanceThresh, R, t, mask, triangulatedPoints)
...
computeCorrespondEpilines(points::Image, whichImage::Int32, F::Image; lines::Image = (CxxMat())) = computeCorrespondEpilines(points, whichImage, F, lines)
...
triangulatePoints(projMatr1::Image, projMatr2::Image, projPoints1::Image, projPoints2::Image; points4D::Image = (CxxMat())) = triangulatePoints(projMatr1, projMatr2, projPoints1, projPoints2, points4D)
...
correctMatches(F::Image, points1::Image, points2::Image; newPoints1::Image = (CxxMat()), newPoints2::Image = (CxxMat())) = correctMatches(F, points1, points2, newPoints1, newPoints2)
...
reprojectImageTo3D(disparity::Image, Q::Image; _3dImage::Image = (CxxMat()), handleMissingValues::Bool = (false), ddepth::Int32 = Int32(-1)) = reprojectImageTo3D(disparity, Q, _3dImage, handleMissingValues, ddepth)
...
estimateAffine3D(src::Image, dst::Image; out::Image = (CxxMat()), inliers::Image = (CxxMat()), ransacThreshold::Float64 = Float64(3), confidence::Float64 = Float64(0.99)) = estimateAffine3D(src, dst, out, inliers, ransacThreshold, confidence)
...
filterHomographyDecompByVisibleRefpoints(rotations::Array{Image, 1}, normals::Array{Image, 1}, beforePoints::Image, afterPoints::Image; possibleSolutions::Image = (CxxMat()), pointsMask::Image = (MatOP)) = filterHomographyDecompByVisibleRefpoints(rotations, normals, beforePoints, afterPoints, possibleSolutions, pointsMask)
...
undistort(src::Image, cameraMatrix::Image, distCoeffs::Image; dst::Image = (CxxMat()), newCameraMatrix::Image = (MatOP)) = undistort(src, cameraMatrix, distCoeffs, dst, newCameraMatrix)
...
initUndistortRectifyMap(cameraMatrix::Image, distCoeffs::Image, R::Image, newCameraMatrix::Image, size::Size, m1type::Int32; map1::Image = (CxxMat()), map2::Image = (CxxMat())) = initUndistortRectifyMap(cameraMatrix, distCoeffs, R, newCameraMatrix, size, m1type, map1, map2)
...
undistortPoints(src::Image, cameraMatrix::Image, distCoeffs::Image; dst::Image = (CxxMat()), R::Image = (MatOP), P::Image = (MatOP)) = undistortPoints(src, cameraMatrix, distCoeffs, dst, R, P)
...
undistortPointsIter(src::Image, cameraMatrix::Image, distCoeffs::Image, R::Image, P::Image, criteria::TermCriteria; dst::Image = (CxxMat())) = undistortPointsIter(src, cameraMatrix, distCoeffs, R, P, criteria, dst)
...
calcOpticalFlowPyrLK(prevImg::Image, nextImg::Image, prevPts::Image, nextPts::Image; status::Image = (CxxMat()), err::Image = (CxxMat()), winSize::Size = (SizeOYXSYXP), maxLevel::Int32 = Int32(3), criteria::TermCriteria = (TermCriteriaOTermCriteriaggCOUNTRTermCriteriaggEPSSGZWSGWUWXP), flags::Int32 = Int32(0), minEigThreshold::Float64 = Float64(1e-4)) = calcOpticalFlowPyrLK(prevImg, nextImg, prevPts, nextPts, status, err, winSize, maxLevel, criteria, flags, minEigThreshold)
