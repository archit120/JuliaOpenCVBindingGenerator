...
mod.add_type<cv::flann::Index>("Index");
    ...
    mod.add_bits<cv::flann::FlannIndexType>("FlannIndexType", jlcxx::julia_type("CppEnum"));   mod.add_type<cvflann::flann_algorithm_t>("cvflann_flann_algorithm_t");
...
;
    ...
    mod.method("jlopencv_cv_flann_cv_flann_Index_cv_flann_Index_knnSearch",  [](cv::flann::Index& cobj, Mat& query, int& knn, Mat& indices, Mat& dists, SearchParams& params) {Mat indices;Mat dists; cobj.knnSearch(query, indices, dists, knn, params);  return make_tuple(move(indices),move(dists));});
    mod.method("jlopencv_cv_flann_cv_flann_Index_cv_flann_Index_knnSearch",  [](cv::flann::Index& cobj, UMat& query, int& knn, UMat& indices, UMat& dists, SearchParams& params) {UMat indices;UMat dists; cobj.knnSearch(query, indices, dists, knn, params);  return make_tuple(move(indices),move(dists));});
    mod.method("jlopencv_cv_flann_cv_flann_Index_cv_flann_Index_radiusSearch",  [](cv::flann::Index& cobj, Mat& query, double& radius, int& maxResults, Mat& indices, Mat& dists, SearchParams& params) {Mat indices;Mat dists; auto retval = cobj.radiusSearch(query, indices, dists, radius, maxResults, params);  return make_tuple(move(retval),move(indices),move(dists));});
    mod.method("jlopencv_cv_flann_cv_flann_Index_cv_flann_Index_radiusSearch",  [](cv::flann::Index& cobj, UMat& query, double& radius, int& maxResults, UMat& indices, UMat& dists, SearchParams& params) {UMat indices;UMat dists; auto retval = cobj.radiusSearch(query, indices, dists, radius, maxResults, params);  return make_tuple(move(retval),move(indices),move(dists));});
