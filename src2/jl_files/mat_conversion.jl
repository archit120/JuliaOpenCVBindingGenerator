...
function julia_to_cpp(img::Mat, expected_type)
...
function julia_to_cpp(var::Array{Mat, 1}, expected_type)
    ret = CxxWrap.StdVector{Mat_}()
    for x in var
...
function cpp_to_julia(var::CxxWrap.StdVector{T}) where {T<:Mat_}
    ret = Array{Mat, 1}()
    for x in var
