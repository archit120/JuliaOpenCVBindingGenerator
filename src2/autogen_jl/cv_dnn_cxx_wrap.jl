...
dnn_getParam(cobj::Net, layer::LayerId; numParam::Int32 = Int32(0)) = dnn_getParam(cobj, layer, numParam)
...
dnn_estimate(cobj::KeypointsModel, frame::InputArray; thresh::Float32 = Float32(0.5)) = dnn_estimate(cobj, frame, thresh)
...
dnn_detect(cobj::DetectionModel, frame::InputArray; confThreshold::Float32 = Float32(0.5), nmsThreshold::Float32 = Float32(0)) = dnn_detect(cobj, frame, confThreshold, nmsThreshold)
...
dnn_blobFromImage(image::InputArray; scalefactor::Float64 = Float64(1.0), size::Size{Int32} = (cpp_to_julia(SizeOP())), mean::Scalar = (cpp_to_julia(ScalarOP())), swapRB::Bool = (false), crop::Bool = (false), ddepth::Int32 = Int32(CV_32F)) = dnn_blobFromImage(image, scalefactor, size, mean, swapRB, crop, ddepth)
...
dnn_blobFromImages(images::Array{InputArray, 1}; scalefactor::Float64 = Float64(1.0), size::Size{Int32} = (cpp_to_julia(SizeOP())), mean::Scalar = (cpp_to_julia(ScalarOP())), swapRB::Bool = (false), crop::Bool = (false), ddepth::Int32 = Int32(CV_32F)) = dnn_blobFromImages(images, scalefactor, size, mean, swapRB, crop, ddepth)
...
dnn_NMSBoxes(bboxes::Array{Rect{Float64}, 1}, scores::Array{Float32, 1}, score_threshold::Float32, nms_threshold::Float32; eta::Float32 = Float32(1), top_k::Int32 = Int32(0)) = dnn_NMSBoxes(bboxes, scores, score_threshold, nms_threshold, eta, top_k)
...
dnn_NMSBoxesRotated(bboxes::Array{RotatedRect, 1}, scores::Array{Float32, 1}, score_threshold::Float32, nms_threshold::Float32; eta::Float32 = Float32(1), top_k::Int32 = Int32(0)) = dnn_NMSBoxesRotated(bboxes, scores, score_threshold, nms_threshold, eta, top_k)
